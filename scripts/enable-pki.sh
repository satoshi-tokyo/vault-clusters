#!/bin/sh
engine=unified
VAULT_ADDR="https://vault-cluster-pri-public-vault-be405a6a.b6266002.z1.hashicorp.cloud:8200"
...
vault write pki-$engine/config/urls \
  issuing_certificates="$VAULT_ADDR/v1/admin/pki-$engine/ca" \
  crl_distribution_points="$VAULT_ADDR/v1/admin/pki-$engine/crl"
...
vault write pki-int-$engine/config/cluster \
  path="${VAULT_ADDR}/v1/admin/pki-int-$engine" \
  aia_path="${VAULT_ADDR}/v1/admin/pki-int-$engine"
